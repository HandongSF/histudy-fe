...
export default function TeamInfoModal({ selectedTeam, closeModal }: TeamInfoModalProps) {
   return (
      <Dialog
                  ...
                  <div className="relative w-full aspect-[16/9] sm:aspect-video">
                     {selectedTeam.thumbnail ? (
                        ...
                        <div className="w-full h-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center rounded-t-lg">
                           <p className="text-slate-500 dark:text-slate-400">이미지 없음</p>
                        </div>
                     )}
                     {/* 닫기 버튼: 이미지 우측 상단에 위치 */}
                     <Button
                  ...
                  <div className="px-6 pt-2 pb-6 flex-grow overflow-y-auto">
                     <div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-4 text-slate-600 dark:text-slate-300">
                        <div className="flex items-center">
                           <Square size={25} className="mr-2 text-black dark:text-white" />
                           <span className="text-xl">Team {selectedTeam.id}</span>
                        </div>
                        <div className="flex items-center">
                           <Users size={18} className="mr-2 text-sky-600 dark:text-sky-400" />
                     ...
                     <div>
                        <h3 className="text-md font-semibold text-slate-700 dark:text-slate-200 mb-1.5">참여 인원:</h3>
