...
import { BookMarked, BookOpenCheck, Fingerprint, GripVertical, Tag, UserCircle, Users } from 'lucide-react';
   ...
   return (
      ...
      >
         <div className="flex items-center space-x-3">
            ...
            <span className="font-semibold text-primary mr-2 text-lg">#{index + 1}</span>
   ...
   return (
      <div className="space-y-8">
         <CardHeader className="p-0 mb-2">
            <CardTitle className="text-2xl">3단계: 신청 내용 확인 및 제출</CardTitle>
            <CardDescription>선택하신 내용을 확인하고, 수업의 우선순위를 조정한 후 제출해주세요.</CardDescription>
         </CardHeader>

         <Card>
            <CardHeader>
               <CardTitle className="text-xl flex items-center">
                  <Users className="h-6 w-6 mr-2 text-primary" /> 선택된 친구 ({friends.length}명)
               </CardTitle>
            </CardHeader>
            <CardContent>
               {friends.length > 0 ? (
         ...
         <Card>
            <CardHeader>
               <CardTitle className="text-xl flex items-center">
                  <BookOpenCheck className="h-6 w-6 mr-2 text-primary" /> 선택된 수업 ({courses.length}개) - 우선순위
                  변경 가능
               </CardTitle>
               <CardDescription>드래그하여 수업의 우선순위를 변경할 수 있습니다. (가장 위가 1순위)</CardDescription>
            </CardHeader>
            <CardContent>
               {courses.length > 0 ? (
