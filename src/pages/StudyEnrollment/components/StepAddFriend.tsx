...
export function StepAddFriends({ selectedFriends, onUpdateFriends }: StepAddFriendsProps) {
   const [searchTerm, setSearchTerm] = useState('');

   const [debouncedSearchTerm] = useDebounce(searchTerm, 250);

   const { data: searchResults } = useQuery(['searchUser', debouncedSearchTerm], () => searchUser(debouncedSearchTerm));
   ...
   const filteredSearchResults = searchResults?.users.filter(
      (friend) => !selectedFriends.some((selectedFriend) => selectedFriend.id === friend.id),
   );
   ...
   return (
      <div className="space-y-6">
         <CardHeader className="p-0 mb-2">
            ...
            <CardDescription>
               스터디를 함께하고 싶은 친구를 이름, 학번, 이메일로 검색하여 최대 {MAX_FRIENDS}명까지 추가하세요.
            </CardDescription>
         </CardHeader>
                     ...
                     >
                        <div className="flex items-center space-x-3">
                           ...
                           <div>
                              ...
                              <p className="text-xs text-muted-foreground">{friend.sid}</p>
                           </div>
                        </div>
                        <Button size="sm" variant="outline" disabled={selectedFriends.length >= MAX_FRIENDS}>
